...
const RadioPrice = ({ radioPriceButtons, price, setPrice }) => {
    return (
        <label htmlFor={`${radioPriceButtons.name}-${radioPriceButtons.price}`} className="block cursor-pointer border border-gray-500 rounded-lg">
            ...
            />
            <div className={`p-4 rounded-lg border-2 ${price.name === radioPriceButtons.name && "border-blue-500 bg-blue-100"} cursor-pointer transition-colors hover:border-blue-500 hover:bg-blue-100`}>
                <div className="flex justify-between items-center">
                    ...
                    <span className="text-gray-600">₹{radioPriceButtons.price}</span>
                </div>
            </div>
        </label>
    );
